...
function command_handler.refresh(_, device)
  ...
  if success then
    ...
    log.trace('Refreshing water sensor ')
    local waterSensor = {}
    waterSensor.value = raw_data.water
    log.info('WaterSensor dump')
    log.info(raw_data.water)
    device:emit_event(caps.waterSensor.water(waterSensor))
  else
...
-- Switch commad
function command_handler.wetdry(device, command)
  log.info('Command ' .. command)
    local waterSensor = {}
    waterSensor.value = command
    device:emit_event(caps.waterSensor.water(waterSensor))
end
------------------------
