...
function command_handler.switch_on(driver, device, command)
  log.debug(string.format("[%s] calling set_power(on)", device.device_network_id))
  device:emit_event(caps.switch.switch.on())
  local switch_data = {}
  switch_data.value = 'on'
  command_handler.send_lan_command(device.device_network_id, 'POST', 'control', switch_data)
end

-- callback to handle an `off` capability command
function command_handler.switch_off(driver, device, command)
  log.debug(string.format("[%s] calling set_power(off)", device.device_network_id))
  device:emit_event(caps.switch.switch.off())
  local switch_data = {}
  switch_data.value = 'off'
  command_handler.send_lan_command(device.device_network_id, 'POST', 'control', switch_data)
end
